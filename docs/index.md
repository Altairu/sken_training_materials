# ようこそ、Sken 技術資料へ


この Wiki は、Skenで実施する各種講習資料や課題をひとまとめにし、効率的に学習できる環境を提供するために作成されました。


## コンテンツのご案内

### 機械班資料
- **[SolidEdge](training_materials/機械班/SolidEdge1.md)**  
  SolidEdgeのインストールから基本操作、ギヤ設計などに関する資料
- **[マシニングセンタ](training_materials/機械班/マシニングセンタ/NVX5060.md)**  
  NVX5060の操作やMCデータ(NCVC)の作り方に関する資料
- **[伝達講習](training_materials/機械班/伝達講習/伝達.md)**  
  動力伝達機構（チェーン、ベルト、ギヤ）の基礎知識とタイミングベルトの設計
- **[軸締結](training_materials/機械班/軸締結/軸締結.md)**  
  軸と部品（プーリ、ギヤ等）の締結手法の比較とノウハウ
- **[歯車講習](training_materials/機械班/歯車講習/歯車.md)**  
  歯車の諸元計算、減速比、バックラッシ等の設計・選定ガイドライン
- **[材料講習](training_materials/機械班/材料講習/材料.md)**  
  ロボット製作に用いる金属・樹脂材料の特性と選定、たわみ計算
- **[リンク講習](training_materials/機械班/リンク講習/リンク.md)**  
  リンク機構の基礎理論と、歩行機構などの実践的な設計手法

### 電気班資料
#### 技術資料

- **[ROS 2](training_materials/講習資料/ROS%202/ROS2_事始.md)**  
  ROS2 の環境構築、基礎から応用、さらにはロボコン向けの実践資料まで

- **[python3](training_materials/講習資料/python3/python3_事始.md)**  
  Python プログラミングの基本操作から実践的なテクニック

- **回路**  
  KiCadの環境構築や、電子回路の設計・解析に必要な知識を図解やサンプルコードを交えて解説  

  - モジュール一覧
  - **新モジュール**
    - **[AltairMD V8](training_materials/講習資料/回路/新モジュール/AltairMD_V8/AltairMD_V8.md)**: モータドライバモジュール V8
    - **[AltairMD V9](training_materials/講習資料/回路/新モジュール/AltairMD_V9/AltairMD_V9.md)**: モータドライバモジュール V9
    - **[ALTAIR MDD V3](training_materials/講習資料/回路/新モジュール/ALTAIR_MDD_V3/ALTAIR_MDD_V3.md)**: マルチデバイスドライバ V3
    - **[ALTAIR SERVO MODULE V6](training_materials/講習資料/回路/新モジュール/ALTAIR_SERVO_MODULE_V6/ALTAIR_SERVO_MODULE_V6.md)**: サーボモジュール V6
    - **[Altair Solenoid Valve V1](training_materials/講習資料/回路/新モジュール/Altair_solenoid_valve_V1/Altair_solenoid_valve_V1.md)**: 電磁弁モジュール V1
  - **旧モジュール**
    - **[AltairMD V7](training_materials/講習資料/回路/旧モジュール/AltairMD_V7/AltairMD_V7.md)**: モータドライバモジュール V7
    - **[ALTAIR MDD V2](training_materials/講習資料/回路/旧モジュール/ALTAIR_MDD_V2/ALTAIR_MDD_V2.md)**: マルチデバイスドライバ V2
    - **[ALTAIR CAN SHIELD V1](training_materials/講習資料/回路/旧モジュール/ALTAIR_CAN_SHIELD%20_V1/ALTAIR_CAN_SHIELD%20_V1.md)**: CANシールド V1
    - **[ALTAIR SERVO MODULE V5](training_materials/講習資料/回路/旧モジュール/ALTAIR_SERVO_MODULE_V5/ALTAIR_SERVO_MODULE_V5.md)**: サーボモジュール V5
    - **[センサモジュール](training_materials/講習資料/回路/旧モジュール/センサモジュール/センサモジュール.md)**: センサモジュール
    - **[電源モジュール](training_materials/講習資料/回路/旧モジュール/電源モジュール/電源モジュール.md)**: 電源モジュール
    - **[電磁弁モジュール](training_materials/講習資料/回路/旧モジュール/電磁弁モジュール/電磁弁モジュール.md)**: 電磁弁モジュール

- **マイコン**  
  各種開発環境やマイコンの使い方に関する資料
  - **[STM32CubeIDE](training_materials/講習資料/STM32CubeIDE/1/環境構築.md)**: 環境構築、ライブラリ・モジュールの使い方、サンプルコードなど
  - **[PlatformIO](training_materials/講習資料/PlatformIO/PlatformIO環境構築/PlatformIOの環境構築.md)**: 環境構築、mbedの使い方、ライブラリの使い方
  - **[SW4STM32](training_materials/講習資料/マイコンSW/マイコン1.md)**: SystemWorkbenchを使った講習資料
  - **[ESP32](training_materials/講習資料/マイコンesp/マイコン1.md)**: ESP32マイコンに関する資料
  - **[CH552](training_materials/講習資料/CH552/CH552_1.md)**: CH552マイコンに関する資料

- **[AIコーディング](training_materials/講習資料/AIコーディング/バイブコーディング.md)**  
  バイブコーディングやGeminiを用いたAIを活用したプログラミング手法

- **その他**  
  - **[1年体験入部](training_materials/講習資料/1年体験入部/S-ken体験入部制御班Lちか.md)**: 体験入部時に使用した資料
  - **[PID制御](training_materials/講習資料/pid/pid.md)**: PID制御に関する資料
  - **[Ubuntu](training_materials/講習資料/ubuntu/ubuntu1.md)**: Ubuntuに関する資料

> **参考:** [qiita](https://qiita.com/_Altair_) の記事もチェックしてください。

[回路モジュールのreadme置き場](https://github.com/SkenHub/Circuit-Module-Readme/tree/main/REDME)はこちら

#### 講習課題

- **[python 課題](training_materials/特訓課題/python.md) / [答え](training_materials/講習資料/code/python答え.md)**  
  実践的な課題とその詳細な解説で、プログラミングスキルを磨きます。
- **[回路課題](training_materials/特訓課題/回路.md)**  
  自身の理解度を確認できる、回路設計に関する特訓課題を提供します。

### 共通資料
- **[wiki使い方](training_materials/共有/wiki_readme.md)**  
  このWikiの編集や運用方法について
- **[マークダウン書き方](training_materials/講習資料/マークダウン書き方.md)**  
  マークダウンの基本的な書き方
- **[ライセンス](training_materials/講習資料/ライセンス/ライセンススライド.md)**  
  ライセンスについて
- **[Github](training_materials/講習資料/github/github1.md)**  
  Githubの使い方

### その他
- **[管理者連絡先](my.md)**  
  お問い合わせ先や管理者に関する情報


## サイトの利用方法

- **ナビゲーションバー:**  
  サイト上部のメニューから各カテゴリの講習資料や課題にワンクリックでアクセス可能です。
- **更新情報:**  
  ページ内の更新履歴で、最新の講習資料や課題の変更点を確認できます。



## コントリビューションについて

みなさんからの資料の追加、修正、誤りの訂正などのフィードバックを歓迎しています。  
GitHub リポジトリ [Altairu/sken_training_materials](https://github.com/Altairu/sken_training_materials) にて、どなたでもコントリビューションが可能です。


## お問い合わせ

ご不明な点やご質問は、以下，もしくは[こちら](https://x.com/Flying___eagle) の DM までお気軽にご連絡ください。

<script src="https://giscus.app/client.js"
        data-repo="Altairu/sken_training_materials"
        data-repo-id="R_kgDOOUfVlQ"
        data-category="General"
        data-category-id="DIC_kwDOOUfVlc4Co0Ge"
        data-mapping="pathname"
        data-strict="0"
        data-reactions-enabled="0"
        data-emit-metadata="0"
        data-input-position="bottom"
        data-theme="preferred_color_scheme"
        data-lang="ja"
        crossorigin="anonymous"
        async>
</script>

---

!!! danger "注意"
    ウェブサイトおよび SNS に記載している情報の正確性については責任を負いかねます。  
    本サイトは情報提供のみを目的としており、各自の責任でご利用ください。  
    また、筆者の所属する組織とは一切関係がありません。

??? Note
    著者:Shion Noguchi