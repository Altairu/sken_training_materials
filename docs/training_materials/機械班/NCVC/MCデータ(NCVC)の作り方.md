...
12. 以下のような，%の下にある２行を消す(**%は絶対に消さないこと**！)．MCは全角文字を処理できないため，消す必要がある．
```gcode
(2025/05/01 13:43)
...
13. その下のG91~Z30.まで(Layer="CAM1" startの前まで)をコピーする．
```gcode
G91G30Z0M192
T18
M06
...
14. (Layer="CAM2" start)とその上のG80との間(G80の下)にM05,M09を入力し，13.でコピーした内容をペーストする．M05とM09は改行すること．(括弧内はコメントアウトなので消すこと)
```gcode
G80 (ここまでがCAM1)
